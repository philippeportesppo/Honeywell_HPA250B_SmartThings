 ...
 */
definition(
		name: "Honeywell HPA250B UPnP Service Manager",
		namespace: "philippeportesppo",
		author: "Philippe Portes",
		description: "Discover and configures your Raspberry Pi bridge to Honeywell HPA250B (see https://github.com/philippeportesppo/Honeywell_HPA250B_SmartThings).",
...
preferences {
	page(name: "searchTargetSelection", title: "UPnP Honeywell HPA250B via Pi Search Target", nextPage: "deviceDiscovery") {
		section("Search Target") {
			input "searchTarget", "string", title: "Search Target", defaultValue: "urn:schemas-upnp-org:device:hpa250b:1", required: true
		}
	}
...
def deviceDiscovery() {
	...
	devices.each {
		def value = it.value.name ?: "HPA250B ${it.value.ssdpUSN.split(':')[1][-3..-1]}"
	...
	return dynamicPage(name: "deviceDiscovery", title: "Discovery Started!", nextPage: "", refreshInterval: 5, install: true, uninstall: true) {
		section("Please wait while we discover your Honeywell HPA250B Device. Discovery can take five minutes or more, so sit back and relax! Select your device below once discovered.") {
...
def addDevices() {
	...
	selectedDevices.each { dni ->
		...
		if (!d) {
			addChildDevice("philippeportesppo", "Honeywell_HPA250B", selectedDevice.value.mac, selectedDevice?.value.hub, [
				"label": selectedDevice?.value?.name ?: "HPA250B",
				"data": [
